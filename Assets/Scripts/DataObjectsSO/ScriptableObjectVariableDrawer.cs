...
// Unity inspector
[CustomPropertyDrawer(typeof(ScriptableObjectVariable))]
public class ScriptableObjectVariableDrawer : PropertyDrawer
{
    public override VisualElement CreatePropertyGUI(SerializedProperty property)
    {
        ...
        {
            objectType = typeof(ScriptableObjectVariable)
        };
        ...
        objectField.RegisterValueChangedCallback(
            ...
            {
                var variable = e.newValue as ScriptableObjectVariable;
                if (variable != null)
                {
                    valueLabel.text = $"Current Value: {variable.Value}";
                    variable.OnValueChanged += (ScriptableObjectBase newValue) => valueLabel.text = $"Current Value: {newValue}";
                }
                else
                {
        ...
        var currentVariable = property.objectReferenceValue as ScriptableObjectVariable;
